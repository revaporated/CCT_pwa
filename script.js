...
let sessionTrialRecords=[];
let signedOutcomeStreak=0;
let trialDataViewRecords=new Map();
const selectedTrialDataSessionIds=new Set();
let showIntervalTiming=false;
let selectedVoice="";
let playbackSpeed=1;
let beepVolume=DEFAULT_BEEP_VOLUME_PERCENT;
let voiceAudioCache={};
let voiceBufferCache={};
let activeStimulusAudios=new Set();
let voiceLibrary={};
let voiceTestInProgress=false;
let beepAudioContext=null;
let stimulusAudioContext=null;
...
function updateAppViews(){
  ...
  document.body.classList.toggle("session-active",sessionVisible);
  startBtn.disabled=sessionState!=="idle";
  answer.disabled=sessionState!=="active";
  endSessionBtn.disabled=!sessionVisible;
  if(typeof keypadButtons!=="undefined"){
    keypadButtons.forEach(button=>{
      const isMinusButton=button.dataset.keypadAction==="minus";
      button.disabled=sessionState!=="active" || (isMinusButton && arithmeticMode!=="subtraction");
    });
  }
}
...
function handleSettingsChange(event){
  ...
  if(target===themeToggle){
    ...
    voiceSelect.value=selectedVoice;
    void preloadVoiceBuffers(selectedVoice);
...
const HISTORY_FILTER_DEFS={
  status:{
    defaultValue:"all",
    values:new Set(["all","Completed","Manually exited","Interrupted"]),
...
function normalizeHistoryRecord(record){
  ...
  const status=record?.status==="Manually exited"
    ? "Manually exited"
...
function normalizeLatestTraceRecord(record){
  ...
  return {
    ...
    status:record?.status==="Manually exited"
      ? "Manually exited"
      : (record?.status==="Interrupted" ? "Interrupted" : "Completed"),
...
function normalizeTrialLogRecord(record){
  const sessionId=typeof record?.sessionId==="string" ? record.sessionId.trim() : "";
  if(!sessionId) return null;

  let streak=0;
  const trials=(Array.isArray(record?.trials) ? record.trials : []).map((trial,index)=>{
    const correct=trial?.correct===true;
    streak=correct
      ? (streak>0 ? streak+1 : 1)
      : (streak<0 ? streak-1 : -1);
    return {
      trialIndex:index+1,
      correct,
      responseTimeMs:Math.max(0,Number(trial?.responseTimeMs)||0),
      intervalMs:Math.max(1,Number(trial?.intervalMs)||0),
      streak
    };
  });
...
const sessionHistoryStore=(()=>{
  ...
  const TRIAL_LOG_STORE_NAME="sessionTrialLogs";
  const ENDED_AT_INDEX_NAME="endedAt";
  const fallbackSessions=[];
  let fallbackLatestTrace=null;
  const fallbackTrialLogs=new Map();
  let fallbackTotals={
    completedSessions:0,
    totalCorrectAnswers:0,
    totalDurationMs:0
  ...
  async function openDb(){
    ...
    const openPromise=new Promise((resolve,reject)=>{
      ...
      request.onupgradeneeded=()=>{
        ...
        if(!db.objectStoreNames.contains(TRIAL_LOG_STORE_NAME)){
          db.createObjectStore(TRIAL_LOG_STORE_NAME,{ keyPath:"sessionId" });
        }
      };
      ...
      request.onblocked=()=>{
        console.warn("CCT data storage is waiting for another open tab to close before it can finish the database update.");
      };
  ...
  async function saveSessionBundle(record,{ latestTraceRecord=null, trialLogRecord=null }={}){
    const normalized=normalizeHistoryRecord(record);
    const normalizedTrace=latestTraceRecord ? normalizeLatestTraceRecord(latestTraceRecord) : null;
    const normalizedTrialLog=trialLogRecord ? normalizeTrialLogRecord(trialLogRecord) : null;
    if(normalizedTrialLog && normalizedTrialLog.sessionId!==normalized.sessionId){
      throw new Error("Trial log session ID does not match the session summary");
    }
    if(normalizedTrialLog){
      fallbackTrialLogs.set(normalizedTrialLog.sessionId,normalizedTrialLog);
    }
    ...
    if(!supportsIndexedDB){
      ...
      if(normalizedTrace) fallbackLatestTrace=normalizedTrace;
      if(normalizedTrialLog) fallbackTrialLogs.set(normalizedTrialLog.sessionId,normalizedTrialLog);
      if(previousDelta){
    ...
    try{
      ...
      const storeNames=[STORE_NAME,TOTALS_STORE_NAME];
      if(normalizedTrace) storeNames.push(TRACE_STORE_NAME);
      if(normalizedTrialLog) storeNames.push(TRIAL_LOG_STORE_NAME);
      const tx=db.transaction(storeNames,"readwrite");
      tx.objectStore(STORE_NAME).put(normalized);
      tx.objectStore(TOTALS_STORE_NAME).put(updatedTotals);
      if(normalizedTrace) tx.objectStore(TRACE_STORE_NAME).put(normalizedTrace);
      if(normalizedTrialLog) tx.objectStore(TRIAL_LOG_STORE_NAME).put(normalizedTrialLog);
      await txDone(tx);
    }catch(e){}
    upsertFallbackSession(normalized);
    if(normalizedTrace) fallbackLatestTrace=normalizedTrace;
  ...
  async function saveSession(record){
    return saveSessionBundle(record);
  }

  async function saveCompletedSession(record,latestTraceRecord,trialLogRecord){
    return saveSessionBundle(record,{ latestTraceRecord, trialLogRecord });
  }
  ...
  async function getTrialLogSessionIds(){
    if(!supportsIndexedDB){
      return [...fallbackTrialLogs.keys()];
    }

    try{
      const db=await openDb();
      const tx=db.transaction(TRIAL_LOG_STORE_NAME,"readonly");
      const request=tx.objectStore(TRIAL_LOG_STORE_NAME).getAllKeys();
      const storedSessionIds=await new Promise((resolve,reject)=>{
        request.onsuccess=()=>resolve(request.result || []);
        request.onerror=()=>reject(request.error || new Error("Failed to read trial-log keys"));
      });
      return [...new Set([...storedSessionIds,...fallbackTrialLogs.keys()])];
    }catch(e){
      return [...fallbackTrialLogs.keys()];
    }
  }

  async function getTrialLogsBySessionIds(sessionIds){
    const ids=[...new Set((Array.isArray(sessionIds) ? sessionIds : []).filter(sessionId=>typeof sessionId==="string" && sessionId))];
    if(!ids.length) return [];
    if(!supportsIndexedDB){
      return ids.map(sessionId=>normalizeTrialLogRecord(fallbackTrialLogs.get(sessionId))).filter(Boolean);
    }

    try{
      const db=await openDb();
      const tx=db.transaction(TRIAL_LOG_STORE_NAME,"readonly");
      const store=tx.objectStore(TRIAL_LOG_STORE_NAME);
      const records=await Promise.all(ids.map(sessionId=>new Promise((resolve,reject)=>{
        const request=store.get(sessionId);
        request.onsuccess=()=>resolve(request.result || null);
        request.onerror=()=>reject(request.error || new Error("Failed to read trial log"));
      })));
      const normalized=records
        .map((record,index)=>normalizeTrialLogRecord(record || fallbackTrialLogs.get(ids[index])))
        .filter(Boolean);
      normalized.forEach(record=>fallbackTrialLogs.set(record.sessionId,record));
      return normalized;
    }catch(e){
      return ids.map(sessionId=>normalizeTrialLogRecord(fallbackTrialLogs.get(sessionId))).filter(Boolean);
    }
  ...
  async function clearAll(){
    ...
    try{
      const db=await openDb();
      const tx=db.transaction([STORE_NAME,TRACE_STORE_NAME,TOTALS_STORE_NAME,TRIAL_LOG_STORE_NAME],"readwrite");
      tx.objectStore(STORE_NAME).clear();
      tx.objectStore(TRACE_STORE_NAME).clear();
      tx.objectStore(TOTALS_STORE_NAME).clear();
      tx.objectStore(TRIAL_LOG_STORE_NAME).clear();
      await txDone(tx);
    }catch(e){}
    fallbackSessions.length=0;
    fallbackLatestTrace=null;
  ...
  async function clearSessionsOnly(){
    ...
    try{
      const db=await openDb();
      const tx=db.transaction([STORE_NAME,TRACE_STORE_NAME,TRIAL_LOG_STORE_NAME],"readwrite");
      tx.objectStore(STORE_NAME).clear();
      tx.objectStore(TRACE_STORE_NAME).clear();
      tx.objectStore(TRIAL_LOG_STORE_NAME).clear();
      await txDone(tx);
  ...
  return {
    ...
    saveLatestTrace,
    getAllSessions,
    getTrialLogSessionIds,
    getTrialLogsBySessionIds,
    getLatestTrace,
...
function normalizeLatestIntervalGroupSize(value,fallback=1){
  const parsed=Math.round(Number(value));
  const fallbackValue=Math.round(Number(fallback));
  const resolved=Number.isFinite(parsed) ? parsed : (Number.isFinite(fallbackValue) ? fallbackValue : 1);
  return Math.max(1,Math.min(50,resolved));
}

function buildLatestIntervalOverviewBlocks(tracePoints,requestedBlockSize=1){
  const pointCount=tracePoints.length;
  const blockSize=normalizeLatestIntervalGroupSize(requestedBlockSize);
...
function syncLatestIntervalGroupControl(hasTrace=true){
  const groupSize=normalizeLatestIntervalGroupSize(latestIntervalChartViewState.groupSize);
  latestIntervalChartViewState.groupSize=groupSize;
  if(latestIntervalGroupSizeInput){
    latestIntervalGroupSizeInput.value=String(groupSize);
    latestIntervalGroupSizeInput.disabled=!hasTrace;
  }
  if(latestIntervalGroupSizeValue){
    latestIntervalGroupSizeValue.value=String(groupSize);
    latestIntervalGroupSizeValue.textContent=String(groupSize);
  }
}
...
function renderLatestIntervalChartEmptyState(message){
  clearChartInteractions(latestIntervalChart);
  syncLatestIntervalGroupControl(false);
...
function renderLatestIntervalChartOverview(latestTrace,overviewData){
  ...
  if(latestIntervalCaption){
    latestIntervalCaption.textContent=overviewData.blockSize===1
      ...
      : `Overview shows groups of up to ${overviewData.blockSize} questions. The final group can be smaller. Tap a group to zoom into its questions.`;
  }
}
...
function renderHistorySessionsSection(viewData){
  ...
  pageData.sessions.forEach(session=>{
    ...
    status.className="history-status " + (session.status==="Completed" ? "completed" : "manual");
...
function renderLatestIntervalChart(latestTrace){
  const tracePoints=Array.isArray(latestTrace?.trace) ? latestTrace.trace : [];
  const sessionChanged=syncLatestIntervalChartSession(latestTrace);
  ...
  if(sessionChanged && !latestIntervalChartViewState.groupSizeIsUserSelected){
    latestIntervalChartViewState.groupSize=normalizeLatestIntervalGroupSize(
      getLatestIntervalBlockSize(tracePoints.length,latestIntervalChart?.clientWidth || 720)
    );
  }
  syncLatestIntervalGroupControl(true);

  const overviewData=buildLatestIntervalOverviewBlocks(tracePoints,latestIntervalChartViewState.groupSize);
...
function downloadTrialLog(record){
  const normalized=normalizeTrialLogRecord(record);
  if(!normalized) return;
  downloadJsonFile(normalized,`${normalized.sessionId}.json`);
}

function updateTrialDataSelectionControls(){
  const visibleIds=[...trialDataViewRecords.keys()];
  const visibleIdSet=new Set(visibleIds);
  [...selectedTrialDataSessionIds].forEach(sessionId=>{
    if(!visibleIdSet.has(sessionId)) selectedTrialDataSessionIds.delete(sessionId);
  });

  const totalCount=visibleIds.length;
  const selectedCount=selectedTrialDataSessionIds.size;
  const allSelected=totalCount>0 && selectedCount===totalCount;
  trialDataSelectAllCheckbox.disabled=totalCount===0;
  trialDataSelectAllCheckbox.checked=allSelected;
  trialDataSelectAllCheckbox.indeterminate=selectedCount>0 && !allSelected;
  selectAllTrialDataBtn.disabled=totalCount===0 || allSelected;
  clearTrialDataSelectionBtn.disabled=selectedCount===0;
  downloadSelectedTrialDataBtn.disabled=selectedCount===0;
  trialDataSelectionSummary.textContent=`${selectedCount} of ${totalCount} sessions selected`;
}

function setAllTrialDataSelected(isSelected){
  selectedTrialDataSessionIds.clear();
  if(isSelected){
    trialDataViewRecords.forEach((record,sessionId)=>{
      selectedTrialDataSessionIds.add(sessionId);
    });
  }
  trialDataTableBody.querySelectorAll('input[type="checkbox"][data-session-id]').forEach(checkbox=>{
    checkbox.checked=isSelected;
  });
  updateTrialDataSelectionControls();
}

function renderTrialDataTable(trialLogSessionIds,sessions){
  const sessionById=new Map((Array.isArray(sessions) ? sessions : []).map(session=>[session.sessionId,session]));
  const tableRecords=[...new Set((Array.isArray(trialLogSessionIds) ? trialLogSessionIds : [])
    .filter(sessionId=>typeof sessionId==="string" && sessionId))]
    .map(sessionId=>({
      sessionId,
      trialCount:Number(sessionById.get(sessionId)?.totalQuestionsAsked)
    }))
    .sort((a,b)=>{
      const aTime=Number(sessionById.get(a.sessionId)?.endedAt)||0;
      const bTime=Number(sessionById.get(b.sessionId)?.endedAt)||0;
      return bTime-aTime || a.sessionId.localeCompare(b.sessionId);
    });

  trialDataViewRecords=new Map(tableRecords.map(record=>[record.sessionId,record]));
  trialDataTableBody.innerHTML="";

  if(!tableRecords.length){
    const row=document.createElement("tr");
    const cell=document.createElement("td");
    cell.colSpan=4;
    cell.className="trial-data-empty-cell";
    cell.textContent="No saved trial logs yet.";
    row.appendChild(cell);
    trialDataTableBody.appendChild(row);
    updateTrialDataSelectionControls();
    return;
  }
  ...
  tableRecords.forEach(record=>{
    ...
    selectCell.className="trial-data-select-cell";
    const checkbox=document.createElement("input");
    checkbox.type="checkbox";
    checkbox.dataset.sessionId=record.sessionId;
    checkbox.checked=selectedTrialDataSessionIds.has(record.sessionId);
    checkbox.setAttribute("aria-label",`Select trial log ${record.sessionId}`);
    checkbox.onchange=()=>{
      if(checkbox.checked){
        selectedTrialDataSessionIds.add(record.sessionId);
      }else{
        selectedTrialDataSessionIds.delete(record.sessionId);
      }
      updateTrialDataSelectionControls();
    };
    ...
    sessionCell.className="trial-data-session-cell";
    const date=document.createElement("span");
    date.className="trial-data-session-date";
    date.textContent=session
      ? formatSessionDateTime(session.endedAt || session.startedAt)
      : "Saved session";
    const sessionId=document.createElement("span");
    sessionId.className="trial-data-session-id";
    sessionId.textContent=record.sessionId;
    sessionCell.appendChild(date);
    sessionCell.appendChild(sessionId);

    const countCell=document.createElement("td");
    countCell.className="trial-data-count-cell";
    countCell.textContent=Number.isFinite(record.trialCount) ? String(record.trialCount) : "—";

    const downloadCell=document.createElement("td");
    downloadCell.className="trial-data-download-cell";
    const downloadButton=document.createElement("button");
    downloadButton.type="button";
    downloadButton.className="secondary-button trial-data-download-button";
    downloadButton.textContent="Download JSON";
    downloadButton.setAttribute("aria-label",`Download trial log ${record.sessionId}`);
    downloadButton.onclick=async()=>{
      downloadButton.disabled=true;
      try{
        const [trialLog]=await sessionHistoryStore.getTrialLogsBySessionIds([record.sessionId]);
        if(trialLog) downloadTrialLog(trialLog);
      }finally{
...
async function downloadSelectedTrialLogs(){
  const selectedSessionIds=[...trialDataViewRecords.keys()]
    .filter(sessionId=>selectedTrialDataSessionIds.has(sessionId));
  if(!selectedSessionIds.length) return;
  downloadSelectedTrialDataBtn.disabled=true;
  try{
    const selectedRecords=await sessionHistoryStore.getTrialLogsBySessionIds(selectedSessionIds);
    if(selectedRecords.length===1){
      downloadTrialLog(selectedRecords[0]);
      return;
    }
    if(selectedRecords.length>1){
      downloadJsonFile({
        schemaVersion:1,
        sessionTrialLogs:selectedRecords.map(normalizeTrialLogRecord).filter(Boolean)
      },`cct-trial-logs-${formatExportFileDate()}.json`);
    }
  }finally{
    updateTrialDataSelectionControls();
  }
...
function renderHistoryView(viewData){
  ...
  const trialLogSessionIds=viewData?.trialLogSessionIds || [];
  const allSessions=viewData?.allSessions || [];
  const shouldRenderTrialData=viewData?.renderTrialData!==false;
  ...
  if(shouldRenderTrialData){
    renderTrialDataTable(trialLogSessionIds,allSessions);
  }
}
...
async function refreshTrialDataTable(){
  const refreshToken=++trialDataRefreshToken;
  try{
    const [trialLogSessionIds,allSessions]=await Promise.all([
      sessionHistoryStore.getTrialLogSessionIds(),
      sessionHistoryStore.getAllSessions()
    ]);
    if(refreshToken!==trialDataRefreshToken) return;
    renderTrialDataTable(trialLogSessionIds,allSessions);
  }catch(e){
    if(refreshToken!==trialDataRefreshToken) return;
    renderTrialDataTable([],[]);
  }
}
...
async function refreshHistoryView(){
  ...
  const trialRefreshToken=++trialDataRefreshToken;
  const filtersSnapshot={ ...historyFilters };
  try{
    await historyTrendUpdateChain;
    const [stats,latestTrace,fallbackSettings,pageData,trialLogSessionIds,allSessions]=await Promise.all([
      sessionHistoryStore.getStats(),
      sessionHistoryStore.getLatestTrace(),
      sessionHistoryStore.getMostRecentHistorySettings(),
      sessionHistoryStore.getSessionPage({
    ...
    renderHistoryView({
      ...
      trialLogSessionIds,
      allSessions,
      renderTrialData:trialRefreshToken===trialDataRefreshToken,
...
async function requestSessionWakeLock(){
  if(sessionState!=="active" || document.visibilityState!=="visible") return;
...
function retainOnlyVoiceCache(voiceKey){
  ...
  Object.keys(voiceBufferCache).forEach(key=>{
    if(key!==voiceKey){
      delete voiceBufferCache[key];
    }
  });
}
...
function getStimulusAudioContext(){
  if(stimulusAudioContext && stimulusAudioContext.state!=="closed") return stimulusAudioContext;
  const AudioContextCtor=window.AudioContext||window.webkitAudioContext;
  if(!AudioContextCtor) return null;
  try{
    stimulusAudioContext=new AudioContextCtor();
  }catch(e){
    stimulusAudioContext=null;
  }
  return stimulusAudioContext;
}

async function resumeStimulusAudioContext(){
  const context=getStimulusAudioContext();
...
function preloadVoiceBuffers(voiceKey){
  const context=getStimulusAudioContext();
  if(!context || typeof window.fetch!=="function") return Promise.resolve(null);
  if(voiceBufferCache[voiceKey]?.ready) return Promise.resolve(voiceBufferCache[voiceKey]);
  if(voiceBufferCache[voiceKey]?.loading) return voiceBufferCache[voiceKey].loading;

  const entry=voiceBufferCache[voiceKey] || { buffers:{}, ready:false, loading:null };
  const clipNumbers=[1,2,3,4,5,6,7,8,9];
  entry.loading=Promise.all(clipNumbers.map(async num=>{
    const response=await fetch(getVoiceClipUrl(voiceKey,num));
    if(!response.ok) throw new Error(`Audio request failed: ${response.status}`);
    const encodedAudio=await response.arrayBuffer();
    entry.buffers[num]=await context.decodeAudioData(encodedAudio);
  })).then(()=>{
    entry.ready=true;
    entry.loading=null;
    voiceBufferCache[voiceKey]=entry;
    return entry;
  }).catch(()=>{
    entry.loading=null;
    voiceBufferCache[voiceKey]=entry;
  ...
  activeStimulusAudios.forEach(audio=>{
    try{
      if(typeof audio.stop==="function"){
        audio.stop();
        if(typeof audio.disconnect==="function") audio.disconnect();
      }else{
        audio.pause();
        audio.currentTime=0;
      }
...
function playStimulusAudio(num){
  ...
  const bufferEntry=voiceBufferCache[voice];
  const context=getStimulusAudioContext();
  const buffer=bufferEntry&&bufferEntry.ready&&bufferEntry.buffers&&bufferEntry.buffers[num];
  ...
  if(context && context.state==="running" && buffer){
    ...
    activeStimulusAudios.add(source);
    source.addEventListener("ended",()=>{
      activeStimulusAudios.delete(source);
      try{
        source.disconnect();
      }catch(e){}
    },{ once:true });
...
function recordSessionTrial(isCorrect,responseTime,trialInterval){
  signedOutcomeStreak=isCorrect
    ? (signedOutcomeStreak>0 ? signedOutcomeStreak+1 : 1)
    : (signedOutcomeStreak<0 ? signedOutcomeStreak-1 : -1);
  sessionTrialRecords.push({
    trialIndex:scoredItemCount,
    correct:isCorrect===true,
    responseTimeMs:Math.max(0,Number(responseTime)||0),
    intervalMs:Math.max(1,Number(trialInterval)||0),
    streak:signedOutcomeStreak
  });
}
...
function finalizeQuestionState(questionState,submittedValue,finalizedAt){
  ...
  recordSessionTrial(isCorrect,responseTime,questionState.responseInterval || interval);
...
async function startGame(){
  ...
  selectedVoice=resolveVoiceKey(voiceSelect.value);
  voiceSelect.value=selectedVoice;
  const audioResumePromise=resumeStimulusAudioContext();
  const audioReadyPromise=Promise.all([
    preloadVoice(selectedVoice),
    preloadVoiceBuffers(selectedVoice)
  ]);
  ...
  showIntervalTiming=showIntervalTimingToggle.checked;
  playbackSpeed=parseFloat(playbackSpeedSelect.value)||1;
  await Promise.all([audioResumePromise,audioReadyPromise]);
...
function stopGame(reason="manual"){
  ...
  const trialLogRecord=buildTrialLogRecord();
  correctResponseTimes=[];
  if(shouldStoreSession(sessionRecord)){
    void sessionHistoryStore.saveCompletedSession(sessionRecord,latestTraceRecord,trialLogRecord).catch(()=>{});
  }
...
function checkInputLive(event,submittedAt=getClockTime()){
  ...
  if(isCorrectAnswerInput(activeQuestionState,submittedValue,submittedAt)){
    finalizeQuestionState(activeQuestionState,submittedValue,submittedAt);
  }
}
...
function enterAnswerFromKeypad({ value=null, action=null, submittedAt=getClockTime() }={}){
  ...
  if(action==="backspace"){
    ...
    if(arithmeticMode!=="subtraction") return;
  ...
  checkInputLive(null,submittedAt);
}
...
const latestIntervalGroupSizeInput=document.getElementById("latestIntervalGroupSize");
const latestIntervalGroupSizeValue=document.getElementById("latestIntervalGroupSizeValue");
...
const selectAllTrialDataBtn=document.getElementById("selectAllTrialDataBtn");
const clearTrialDataSelectionBtn=document.getElementById("clearTrialDataSelectionBtn");
const downloadSelectedTrialDataBtn=document.getElementById("downloadSelectedTrialDataBtn");
...
latestIntervalGroupSizeInput.oninput=()=>{
  const groupSize=normalizeLatestIntervalGroupSize(latestIntervalGroupSizeInput.value);
  latestIntervalChartViewState.groupSize=groupSize;
  latestIntervalChartViewState.groupSizeIsUserSelected=true;
  latestIntervalGroupSizeValue.value=String(groupSize);
  latestIntervalGroupSizeValue.textContent=String(groupSize);
};
latestIntervalGroupSizeInput.onchange=()=>{
  clearChartInteractions(latestIntervalChart);
  setLatestIntervalChartMode("overview",null);
  if(latestHistoryChartContext.latestTrace){
    renderLatestIntervalChart(latestHistoryChartContext.latestTrace);
...
refreshSessionsBtn.onclick=()=>{
  ...
  void refreshTrialDataTable();
};
selectAllTrialDataBtn.onclick=()=>{
  setAllTrialDataSelected(true);
};
clearTrialDataSelectionBtn.onclick=()=>{
  setAllTrialDataSelected(false);
};
downloadSelectedTrialDataBtn.onclick=()=>{
  void downloadSelectedTrialLogs();
};
trialDataSelectAllCheckbox.onchange=()=>{
  setAllTrialDataSelected(trialDataSelectAllCheckbox.checked);
};
...
keypadButtons.forEach(button=>{
  button.addEventListener("pointerdown",event=>{
    ...
    enterAnswerFromKeypad({
      value:button.dataset.keypadValue ?? null,
      action:button.dataset.keypadAction ?? null,
      submittedAt:getClockTime()
    });
  });
...
document.addEventListener("keydown",event=>{
  if(sessionState!=="active" || event.repeat) return;
  if(/^\d$/.test(event.key)){
    event.preventDefault();
    enterAnswerFromKeypad({ value:event.key, submittedAt:getClockTime() });
    return;
  }
  if(event.key==="Backspace"){
    event.preventDefault();
    enterAnswerFromKeypad({ action:"backspace", submittedAt:getClockTime() });
    return;
  }
  if(event.key==="-" || event.key==="Subtract"){
    event.preventDefault();
    enterAnswerFromKeypad({ action:"minus", submittedAt:getClockTime() });
  }
});
...
document.addEventListener("visibilitychange",()=>{
  if(document.visibilityState==="hidden" && sessionState==="active"){
    stopGame("interrupted");
    return;
  }
  if(document.visibilityState==="hidden" && sessionState==="starting"){
    currentSessionId="";
